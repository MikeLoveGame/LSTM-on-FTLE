...
class Field():
    ...
    def __init__(self, mapU : np.ndarray, mapV: np.ndarray, time : int, num_partical : int, interpolate_factor : int):
        ...
        for particleslist in self.particles:
            for particle in particleslist:
                path = particle.path
                if (path[0] != path[59]):
                    print("found a moving particle")
        self.ftle = self.calc_ftle(delta = 1, shape=(shape, shape), start = 1, end = 58)

        showFTLE(self.ftle)

        np.save(r"/home/shike/AI/SAM/LCS/data/ftle.npy", self.ftle)
        print(self.ftle)
    ...
    def calc_ftle(self, delta: int, shape: tuple, start: int, end: int):  # its all about tracing the end points and evaluate it by linearize the whole thing which is stupid but fast
        ...
        for i in range(shape[0]-1):
            for j in range(shape[1]-1):
                ...
                jacob[0][0] = (t2[0] - t1[0]) / (o2[0] - o1[0])
                jacob[1][0] = (t2[1] - t1[1]) / (o2[0] - o1[0])
                jacob[0][1] = (t4[0] - t3[0]) / (o4[1] - o3[1])
                jacob[1][1] = (t4[1] - t3[1]) / (o4[1] - o3[1])
                ...
                if (lambdas == 'nan'):
                    ...
                    ftle[i][j] = np.log(np.sqrt(max(lambdas)))

                print(f"ftle at {i} {j} completed")
        return ftle
